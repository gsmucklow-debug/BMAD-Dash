...
export async function updateBadges(containerId, storyId, projectRoot) {
    ...
    try {
        // Fetch both evidences in parallel
        const [gitRes, testRes] = await Promise.all([
        ...
        renderBadges(container, gitData, testData, storyId, projectRoot);
 ...
 */
function renderBadges(container, gitData, testData, storyId, projectRoot) {
    container.innerHTML = '';
    ...
    // VALIDATED BADGE (if both green and recent)
    if (isValidated(gitData, testData)) {
        const validatedBadge = document.createElement('div');
        validatedBadge.className = 'flex items-center px-2 py-1 rounded text-xs font-medium bg-green-900/20 text-green-400 border border-green-900/50';
        validatedBadge.innerHTML = `
            ...
            VALIDATED
        `;
        container.appendChild(validatedBadge);
    }
}
